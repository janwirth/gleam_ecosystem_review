...
| Consistent vertical spacing in a reading column (docs, blog, long-form) | Pick one unit = body line-height; space blocks with `margin-block-start: 1lh` (or `1rlh`) via a `.flow > * + *` rule; keep all line-heights integer multiples of the base at the sizes you actually use | [Vertical rhythm](#vertical-rhythm--the-practice-and-the-debate) |
...
- Imperavi's *User Interface Typography* ([Vertical rhythm chapter](https://imperavi.com/books/ui-typography/principles/vertical-rhythm/)) draws the line by *text size and role*: *"It's worth paying more attention to the vertical rhythm for larger texts"* and *"Vertical rhythm is less important in such interface modules as cards and other small forms."* Its worked sizes are 21px for long reads vs 15px for interface text, and its recommendation for a product that has both is **two separate spacing scales** — line-height multiples for the reading column, a 4/5/8px unit scale for components — rather than one grid forced over everything.
...
**What it converges on:** (a) a strict baseline grid is not worth chasing in CSS — half-leading, images and mixed fonts fight you at every turn; (b) *line-height-multiple spacing* ("rhythm lite") is cheap, and `lh`/`rlh` + `.flow` make it a dozen lines; (c) for product UI, a **4/8px spacing scale with line-heights on the same grid** — what Material, Tailwind, Fluent and Apple all ship — gives most of the visual consistency with none of the fragility; (d) **rhythm is a property of the reading column, not the app shell** — apply it to body copy at 16–21px where the eye tracks line after line, and run components (cards, forms, toolbars, nav) on the unit scale. A product with both keeps two scales and doesn't try to reconcile them. The [design-system table](#type-scales--ratios-and-what-real-design-systems-actually-ship) below shows how few systems actually tie spacing to line-height.
...
| Product UI with a design system (buttons, labels, cards) | 4/8px spacing scale + hand-tuned line-heights on the 4px grid (Material/Fluent/Polaris pattern) + **`text-box: trim-both cap alphabetic`** behind `@supports` | A page-level rhythm fights component padding; cards and small forms don't benefit from it (Imperavi); trimming makes 12px mean 12px. |
...
- Reader-supplied source: Imperavi, *User Interface Typography*, Vertical rhythm chapter (the "rhythm is for larger texts, not cards/small forms" position).
